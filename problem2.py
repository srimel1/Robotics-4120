...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if command == "stop":
            ...
            if my_distance_sensor.read_mm() < 500 and taken is False: 
                ...
                img.save("problem2_{}.jpg".format(number))
                number+=1
                taken = True
...
def Main(trigger):
    ...
    menu_order = ["<LEFT>", "<RIGHT>", "<UP>", "<DOWN>", "<SPACE>", "w"] # and the order of these optionss
